...
export default function FiltrosTablaMisTurnos({
  state,
  setState,
  handle,
  styles,
  ...
  return (
    <div className={`${styles ? styles : "w-full  "}`}>
      ...
      >
        <label className="mr-4 font-medium text-blue">Filtrar por dia:</label>
        <button
          onClick={() => handle(-1)}
        ...
        <button
          onClick={handleToday}
          className="px-2 py-1 ml-5 text-lg font-medium capitalize transition-all duration-200 border border-gray-300 rounded text-blue bg-lightGray hover:bg-gray-300"
        >
          hoy
        </button>
      </div>
