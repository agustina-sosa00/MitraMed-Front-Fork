...
export const tipoDe = (idtrat: number) => (idtrat >= 100 ? idtrat % 100 : idtrat);
export const fillHex = (idtrat: number) => (isARealizar(idtrat) ? "#0369a1" : "#ef4444");
...
export const sinProvisoriosDeTratamientosConCara = (m: TeethIdsState): TeethIdsState => {
  ...
  for (const key of Object.keys(m)) {
    ...
    const filtrado = arr.filter(([cara, trat]) => {
      ...
      const requiereCara = base === TRAT.SELLADO || base === TRAT.RESTAURACIONES;
